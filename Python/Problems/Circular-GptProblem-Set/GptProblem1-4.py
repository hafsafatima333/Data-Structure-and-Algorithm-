...
class Ring:
    ...
    def merge(self, other):
        # agar koi bhi list empty ho
        if self.head is None:
            return other
        if other.head is None:
            return self

        # dono lists ke last nodes nikal lo
        last1 = self._get_last()
        last2 = other._get_last()

        # unke next pointers update karo
        last1.next = other.head
        last2.next = self.head

        return self  # merged list return karo
...
print("Merged Circular Linked List:", merged)
